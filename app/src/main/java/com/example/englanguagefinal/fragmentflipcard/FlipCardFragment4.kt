...
import com.example.englanguagefinal.databinding.FragmentFlipCard4Binding
...
class FlipCardFragment4 : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.mButtonSpeak.setOnClickListener {
            val text: String? = viewModelApp.mVocabulary.value?.success?.get(18)?.word
            speak(text!!) }
        binding.mButtonSpeak1.setOnClickListener {
            val text1: String? = viewModelApp.mVocabulary.value?.success?.get(19)?.word
            speak(text1!!) }
        binding.mButtonSpeak2.setOnClickListener {
            val text2: String? = viewModelApp.mVocabulary.value?.success?.get(20)?.word
            speak(text2!!) }
        binding.mButtonSpeak3.setOnClickListener {
            val text3: String? = viewModelApp.mVocabulary.value?.success?.get(21)?.word
            speak(text3!!) }
        binding.mButtonSpeak4.setOnClickListener {
            val text4: String? = viewModelApp.mVocabulary.value?.success?.get(22)?.word
            speak(text4!!) }
        binding.mButtonSpeak5.setOnClickListener {
            val text5: String? = viewModelApp.mVocabulary.value?.success?.get(23)?.word
            speak(text5!!) }
